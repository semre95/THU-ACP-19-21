...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Asssıgnment4", nullptr, nullptr);
    ...
    for (int i = 0; i < sf_num; ++i)
    {
        snowflakes.push_back(SnowFlake(0.5, glm::vec3(rand()%(WIDTH), rand() % HEIGHT, 0),
                                       glm::vec3(0.0, rand() % 7 * -1, 0.0), glm::vec3(0.0, -0.5, 0.0), (double)rand() / (double)RAND_MAX * 10 + 42, 10.0f + rand() % 60));
    ...
    GLuint texture = loadTexture("snow.png");
    GLuint background_texture = loadTexture("background.jpg");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (rand() % (200) == 0) {
            sf_num++;
            snowflakes.push_back(SnowFlake(0.5, glm::vec3(rand() % (WIDTH), rand() % HEIGHT + 10, 0),
                                           glm::vec3(0.0, rand() % 7 * -1, 0.0), glm::vec3(0.0, -0.5, 0.0), (double)rand() / (double)RAND_MAX * 10 + 42, 10.0f + rand() % 60));
        }
        ...
        for (int i = 0; i < sf_num; ++i)
            snowflakes[i].process(deltaTime*5, glm::vec3(rand()%(WIDTH), HEIGHT + 10, 0)); // apply gravity and update speed, position
