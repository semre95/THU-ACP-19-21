...
int main()
{
    ...
    // Load font as face
    FT_Face face;
    if (FT_New_Face(ft, "arial.ttf", 0, &face)) { // only absolute path works
    ...
    bool res = loadOBJ("planet.obj", vertices1, uvs, normals);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            planets[i].angle = planets[i].angle > 2 * M_PI ? planets[i].angle + M_PI * speed / log(i +2) - 2 * M_PI : planets[i].angle + M_PI * speed / log(i +2);
            glBindVertexArray(VAO);
        ...
        model_vertex = glm::translate(model_vertex,
                                      glm::vec3(planets[2].distance * sin(planets[2].angle) + planets[2].distance / 5 * cos(planets[2].angle * 30), 0.0f, planets[2].distance * cos(planets[2].angle) + planets[2].distance / 5 * sin(planets[2].angle * 30)));
